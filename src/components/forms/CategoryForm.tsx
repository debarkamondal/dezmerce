        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="mx-auto absolute">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Input className="capitalize" placeholder="Enter Category" value={category} onChange={(e) => setCategory(e.target.value)} />
                    <Button disabled={isLoading || (category === initCategory && !catImg)} type="button" onClick={handleClick}>{isLoading ? <Image src="/spinner.svg" height={20} width={20} alt="loading spinner" className="invert" /> : initCategory ? "UpdateCategory" : "Add Category"}</Button>
                </div>
            </DialogContent>
        </Dialog>
    )
